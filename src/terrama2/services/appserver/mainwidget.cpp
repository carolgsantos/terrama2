    ...
    uiSettings(new Ui::SettingsWidget)
{
    ...
    ui->labelLogo->setPixmap(QPixmap (terrama2::core::FindInTerraMA2Path("share/terrama2/icons/app/terrama2_logo.png").c_str()));
...
void MainWidget::setupSystemTrayIcon()
{
    ...
    trayIcon->setIcon(QIcon (terrama2::core::FindInTerraMA2Path("share/terrama2/icons/app/terrama2_logo2.icns").c_str()));
...
{
    QString statusScriptPath = terrama2::core::FindInTerraMA2Path("scripts/start.sh").c_str();
...
{
    QString statusScriptPath = terrama2::core::FindInTerraMA2Path("scripts/stop.sh").c_str();
    terraMA_proc->start(statusScriptPath);
    if (!terraMA_proc->waitForStarted())
            ui->textOutput->insertPlainText("Cannot connect to service.");
}
...
void MainWidget::updateServiceStatus()
{
    if(checkServiceStatus())
    {
        QPixmap statusPixmap = QPixmap (terrama2::core::FindInTerraMA2Path("share/terrama2/icons/app/cancel.png").c_str());
        ...
        if(check)
            ui->labelStatusText->setText("Play to Initialize");
        else
            ui->labelStatusText->setText("Not Running");

    }
    else
    {
        QPixmap statusPixmap = QPixmap (terrama2::core::FindInTerraMA2Path("share/terrama2/icons/app/ok.png").c_str());
        status->setIcon(statusPixmap);
        status->setText("Running");
        ui->labelStatusIcon->setPixmap(statusPixmap);
        ui->labelStatusText->setText("Running");
